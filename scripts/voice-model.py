...
def record_audio(filename="input.wav", samplerate=16000):
    ...
    listener = None
    # Declare listener in outer scope
    ...
    with sd.InputStream(
        samplerate=samplerate, channels=1, dtype="int16", callback=audio_callback
    ):
...
def speak_text(text):
    ...
    os.system(
        "afplay output.wav" if os.name == "posix" else "start output.wav"
    )  # macOS vs Windows
